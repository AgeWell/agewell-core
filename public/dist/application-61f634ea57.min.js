!function(window){"use strict";function registerModule(moduleName,dependencies){angular.module(moduleName,dependencies||[]),angular.module(applicationModuleName).requires.push(moduleName)}var applicationModuleName="agewell",service={applicationEnvironment:window.env,applicationModuleName:applicationModuleName,applicationModuleVendorDependencies:["ngAnimate","ngFileUpload","ngMessages","ngResource","ui.bootstrap","ui.select","ui.router","ui-notification"],registerModule:registerModule};window.ApplicationConfiguration=service,angular.module("ui-notification").config(["NotificationProvider",function(NotificationProvider){NotificationProvider.setOptions({delay:2e3,startTop:20,startRight:10,verticalSpacing:20,horizontalSpacing:20,positionX:"right",positionY:"bottom"})}])}(window);
...
!function(){"use strict";function menuConfig(menuService,Authentication){var profileEdit="settings.profile";null!==Authentication.user&&-1!==Authentication.user.roles.indexOf("volunteer")&&(profileEdit=Authentication.user.hasOwnProperty("volunteer")?"settings.volunteer.edit({volunteerId: '"+Authentication.user.volunteer+"'})":"settings.volunteer.create"),menuService.addMenu("account",{roles:["user"]}),menuService.addMenuItem("account",{title:"",state:"settings",type:"dropdown",roles:["user"]}),menuService.addSubMenuItem("account","settings",{title:"Edit Profile",state:profileEdit,roles:["user"]}),menuService.addSubMenuItem("account","settings",{title:"Edit Profile Picture",state:"settings.picture",roles:["user"]})}angular.module("core").run(menuConfig),menuConfig.$inject=["menuService","Authentication"]}();
...
!function(){"use strict";function ClientsListController($filter,$state,Notification,coreService,ClientsService){function toggle(field,client){function successCallback(res){Notification.info({message:"Update successful!"})}function errorCallback(res){vm.error=res.data.message,client[field]=!client[field]}client[field]=!client[field],client.$update(successCallback,errorCallback)}function newOrder(client){client.lastOrdered===vm.options.order[0]?$state.go("order.edit",{clientId:client.id,orderId:client.lastOrder}):$state.go("order.create",{clientId:client.id})}function buildPager(){vm.pagedItems=[],vm.itemsPerPage=10,vm.currentPage=1,vm.figureOutItemsToDisplay()}function figureOutItemsToDisplay(){vm.filteredItems=$filter("filter")(vm.clients,{$:vm.search}),vm.filterLength=vm.filteredItems.length;var begin=(vm.currentPage-1)*vm.itemsPerPage,end=begin+vm.itemsPerPage;vm.pagedItems=vm.filteredItems.slice(begin,end)}function pageChanged(){vm.figureOutItemsToDisplay()}var vm=this;vm.options=coreService.getOptions("Order"),vm.toggle=toggle,vm.newOrder=newOrder,vm.buildPager=buildPager,vm.figureOutItemsToDisplay=figureOutItemsToDisplay,vm.pageChanged=pageChanged,ClientsService.query(function(data){vm.clients=data,vm.buildPager()})}angular.module("clients").controller("ClientsListController",ClientsListController),ClientsListController.$inject=["$filter","$state","Notification","coreService","ClientsService"]}();
...
!function(){"use strict";function OrdersListController(OrdersService,$filter,$stateParams){function buildPager(){vm.pagedItems=[],vm.itemsPerPage=3,vm.currentPage=1,vm.figureOutItemsToDisplay()}function figureOutItemsToDisplay(){vm.filteredItems=vm.orders,vm.filterLength=vm.filteredItems.length;var begin=(vm.currentPage-1)*vm.itemsPerPage,end=begin+vm.itemsPerPage;vm.pagedItems=vm.filteredItems.slice(begin,end)}function pageChanged(){vm.figureOutItemsToDisplay()}var vm=this;vm.buildPager=buildPager,vm.figureOutItemsToDisplay=figureOutItemsToDisplay,vm.pageChanged=pageChanged,vm.clientid=$stateParams.clientId,OrdersService.query({clientId:vm.clientid},function(data){vm.orders=data,vm.buildPager()})}angular.module("groceries.orders").controller("OrdersListController",OrdersListController),OrdersListController.$inject=["OrdersService","$filter","$stateParams"]}();
...
!function(){"use strict";function UserListController($scope,$filter,Notification,AdminService){function toggle(field,user){function successCallback(res){Notification.info({message:"Update successful!"})}function errorCallback(res){vm.error=res.data.message,user[field]=!user[field]}user[field]=!user[field],user.$update(successCallback,errorCallback)}function buildPager(){vm.pagedItems=[],vm.itemsPerPage=10,vm.currentPage=1,vm.figureOutItemsToDisplay()}function figureOutItemsToDisplay(){vm.filteredItems=$filter("filter")(vm.users,{$:vm.search}),vm.filterLength=vm.filteredItems.length;var begin=(vm.currentPage-1)*vm.itemsPerPage,end=begin+vm.itemsPerPage;vm.pagedItems=vm.filteredItems.slice(begin,end)}function pageChanged(){vm.figureOutItemsToDisplay()}var vm=this;vm.toggle=toggle,vm.buildPager=buildPager,vm.figureOutItemsToDisplay=figureOutItemsToDisplay,vm.pageChanged=pageChanged,AdminService.query(function(data){vm.users=data,vm.buildPager()})}angular.module("users.admin").controller("UserListController",UserListController),UserListController.$inject=["$scope","$filter","Notification","AdminService"]}();
