...
const GroceriesToGo = mongoose.model('GroceriesToGo');
...
exports.create = function(req, res) {
  let groceriesToGo = new GroceriesToGo(req.body);
...
exports.list = function(req, res) {
  GroceriesToGo.find().sort('-created').populate('user', 'displayName').exec(function(err, groceriesToGos) {
    if (err) {
      return res.status(400).send({
        message: errorHandler.getErrorMessage(err)
      });
...
exports.groceriesToGoByID = function(req, res, next, id) {
  ...
  GroceriesToGo.findById(id).populate('user', 'displayName').exec(function (err, groceriesToGo) {
    if (err) {
      return next(err);
    }
    if (!groceriesToGo) {
